...
function TodoCard({ data, handleEdit, handleDelete }) {
  const { _id, title, description, priority, status, shared } = data;
  ...
  function handleShare(todoId) {
    ...
    axios
      .put(`${import.meta.env.VITE_API_URL}/api/todo/share/${todoId}`, { email })
        ...
        <div className="d-flex justify-content-between align-items-start">
          <div>
            ...
            <div className="d-flex flex-wrap gap-2 mb-2">
              ...
              {shared && <span className="badge bg-info px-2 py-1">Shared</span>}
            </div>
          </div>
          <div className="dropdown">
            <button className="btn btn-sm btn-light border-0 text-secondary" data-bs-toggle="dropdown">
...
export function ShowTodoList({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    .then((res) => {
      const updated = res.data.map(t => ({ ...t, shared: t.userId !== user._id }));
      setTodo(updated);
    })
      ...
      <div className="row mb-4 p-4 rounded" style={{ backgroundColor: "#f0f0f7" }}>
        {["Completed", "In Progress", "Pending"].map((status, i) => (
          <div key={i} className="col-6 col-md-3 py-2">
            <div className="p-3 rounded text-white d-flex justify-content-between align-items-center"
              style={{ background: "linear-gradient(135deg, #8e2de2, #4a00e0)" }}>
              <div>
                <div className="fw-bold">{status}</div>
                <div>{todo.filter((t) => t.status === status).length}</div>
              </div>
              <i className={`bi bi-${status === "Completed" ? "check-circle" : status === "In Progress" ? "clock" : "hourglass-split"} fs-3 opacity-50`}></i>
            </div>
          </div>
        ))}
        <div className="col-6 col-md-3 py-2">
          <div className="p-3 rounded text-white d-flex justify-content-between align-items-center"
            style={{ background: "linear-gradient(135deg, #8e2de2, #4a00e0)" }}>
            <div>
              <div className="fw-bold">Total Tasks</div>
              <div>{todo.length}</div>
            </div>
            <i className="bi bi-list-task fs-3 opacity-50"></i>
          </div>
        </div>
      </div>
          ...
          <div className="text-center text-muted">
            {filter === "All" ? (
              <>
                ...
                <Link to="/create-todo" className="btn btn-outline-primary">Create Task</Link>
              </>
            ) : (
